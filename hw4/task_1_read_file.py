...
def read_magic_number(path: str) -> bool:
    ...
    if not os.path.isfile(path=path): 
        raise FileNotFoundError('File not found')         # in case if file not exists

    with open(path, 'r') as f_o:
        data = f_o.read().splitlines()

        try:
            first_line = int(data[0])
        except ValueError: 
            raise ValueError('First line isnt int')                 # in case if first line isnt int value

        if first_line in range(1,3):
            return True
        else:
            return False
